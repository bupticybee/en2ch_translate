...
src_inv_size_base = 80000#len(ind2en) + 3
target_inv_size_base = 80000#len(ind2ch) + 3
...
for i in range(i_save,n_epoch):
    ...
    for j in range(worksum):
        ...
        if one_epoch > 2:
            real_lr = lr / 2
        if one_epoch > 2 and one_batch >= worksum / 2:
            real_lr = lr / 4
        if one_epoch > 3:
            real_lr = lr / 8
        if one_epoch > 3 and one_batch >= worksum / 2:
            real_lr = lr / 16
        if one_epoch > 4:
            real_lr = lr / 32
        if one_epoch > 4 and one_batch >= worksum / 2:
            real_lr = lr / 64
        _, loss = session.run([optimizer,train_loss],feed_dict={x:batch_x,y:batch_y,x_len:lx,y_len:ly,learning_rate:real_lr,y_in:
                                                                np.concatenate((
                                                                np.ones((batch_y.shape[0],1),dtype=np.int) * ch2ind['<go>'],batch_y[:,:-1]) ,axis=1)
                                                                ,y_real_len:by,
                                                                x_real_len:bx,
        ...
        pb.info = "iter {} loss:{} lr:{}".format(i + 1,exp_loss,real_lr)
        ...
        if j % val_step == 0 and j != 0:
            ...
            with open('eval/{}/{}_{}_source_train'.format(model_path,i + 1,j),'w',encoding='utf-8') as whdl:
                for line in train_source_texts:
                    whdl.write("{}\n".format(line))
            print("\niter {} step {} train loss {} train acc {} test loss {} test acc {} bleu {} lr {}\n".format(i+1,j,np.average(train_loss_list[-val_step:]),train_acc,test_loss,test_acc,bleu_score,real_lr))
